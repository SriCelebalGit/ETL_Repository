...
# MAGIC ## customer — CSV
# MAGIC
# MAGIC Written with plain Python file IO, which works on `/Volumes/...` paths from
# MAGIC serverless compute. Row 1003's address is quoted and contains both a comma and a
# MAGIC newline, which is what `multiLine: true` and `escape` in the bronze control row are
# MAGIC there to handle.
...
def customer_rows_batch_2():
    """A later extract: 1002 is promoted to ENTERPRISE, and 1011 is new.

    Re-running the pipeline after this shows SCD2 close 1002's first version and open a
    second one, while the eight unchanged customers produce no new versions at all.
    """
...
rows = customer_rows_batch_1() if batch == "1" else customer_rows_batch_2()
...
log.info("customer CSV written", file=customer_file, row_count=len(rows))

# COMMAND ----------

# MAGIC %md
# MAGIC ## sales_order — JSON Lines
# MAGIC
# MAGIC One JSON object per line, which is why the bronze control row sets
...
def order_rows_batch_1():
    ...
    return [
        {"order_id": 2001, "customer_id": 1001, "order_status": "DELIVERED", "order_date": d(9),
         "delivery_date": d(6), "order_amount": 12500.00, "currency_code": "GBP", "last_modified_ts": ts},
        {"order_id": 2002, "customer_id": 1002, "order_status": "CONFIRMED", "order_date": d(7),
         "delivery_date": None, "order_amount": 3400.50, "currency_code": "USD", "last_modified_ts": ts},
        {"order_id": 2003, "customer_id": 1003, "order_status": "CANCELLED", "order_date": d(6),
         "delivery_date": None, "order_amount": 890.00, "currency_code": "INR", "last_modified_ts": ts},
        # negative amount -> DQ_POSITIVE (drop)
        {"order_id": 2004, "customer_id": 1001, "order_status": "NEW", "order_date": d(5),
         "delivery_date": None, "order_amount": -220.00, "currency_code": "GBP", "last_modified_ts": ts},
        # unlisted currency -> DQ_IN_LIST (drop)
        {"order_id": 2005, "customer_id": 1009, "order_status": "SHIPPED", "order_date": d(4),
         "delivery_date": None, "order_amount": 7700.00, "currency_code": "JPY", "last_modified_ts": ts},
        # dated tomorrow -> DQ_NOT_FUTURE_DATED (drop)
        {"order_id": 2006, "customer_id": 1010, "order_status": "NEW",
         "order_date": (TODAY + timedelta(days=1)).isoformat(),
         "delivery_date": None, "order_amount": 5100.00, "currency_code": "EUR", "last_modified_ts": ts},
        # DELIVERED with no delivery_date -> DQ_NOT_NULL, filtered to delivered (warning)
        {"order_id": 2007, "customer_id": 1002, "order_status": "DELIVERED", "order_date": d(3),
         "delivery_date": None, "order_amount": 2300.00, "currency_code": "USD", "last_modified_ts": ts},
        # delivered BEFORE ordered -> DQ_COMPARE_COLUMNS (drop)
        {"order_id": 2008, "customer_id": 1003, "order_status": "DELIVERED", "order_date": d(2),
         "delivery_date": d(5), "order_amount": 1450.00, "currency_code": "INR", "last_modified_ts": ts},
        # customer 9999 is not in the customer feed -> DQ_REFERENCE_EXISTS (warning)
        {"order_id": 2009, "customer_id": 9999, "order_status": "NEW", "order_date": d(1),
         "delivery_date": None, "order_amount": 640.00, "currency_code": "GBP", "last_modified_ts": ts},
        # DRAFT: removed by the silver row's filter_condition, not by DQ
        {"order_id": 2010, "customer_id": 1010, "order_status": "DRAFT", "order_date": d(1),
         "delivery_date": None, "order_amount": 999.00, "currency_code": "GBP", "last_modified_ts": ts},
    ]


def order_rows_batch_2():
    """Restates order 2002 as DELIVERED with a later last_modified_ts.

    Because the bronze row uses write_mode: merge on order_id with sequence_by
    last_modified_ts, this replaces the earlier version rather than appending.
    """
    ts = (NOW + timedelta(hours=1)).isoformat(sep=" ")
    return [
        {"order_id": 2002, "customer_id": 1002, "order_status": "DELIVERED",
         "order_date": (TODAY - timedelta(days=7)).isoformat(),
         "delivery_date": (TODAY - timedelta(days=1)).isoformat(),
         "order_amount": 3400.50, "currency_code": "USD", "last_modified_ts": ts},
        {"order_id": 2011, "customer_id": 1011, "order_status": "NEW",
         "order_date": TODAY.isoformat(), "delivery_date": None,
         "order_amount": 18250.00, "currency_code": "GBP", "last_modified_ts": ts},
    ]


orders = order_rows_batch_1() if batch == "1" else order_rows_batch_2()
...
log.info("sales_order JSON written", file=order_file, row_count=len(orders))

# COMMAND ----------

# MAGIC %md
# MAGIC ## interaction_event — Parquet
# MAGIC
# MAGIC Written with Spark rather than by hand, because Parquet is a binary format. The
# MAGIC bronze control row reads this directory with `source_file_type: parquet`.

from pyspark.sql import Row  # noqa: E402

event_seed = [
    (1001, "CALL", 0), (1001, "EMAIL", 1), (1002, "WEB_VISIT", 1),
    (1003, "CHAT", 2), (1009, "MEETING", 2), (1010, "EMAIL", 3),
    (1002, "CALL", 3), (1010, "WEB_VISIT", 4),
...
events = [
    ...
    for index, (customer_id, event_type, days_ago) in enumerate(event_seed)
]
...
log.info("interaction_event parquet written", directory=EVENT_DIR, row_count=len(events))
...
for label, directory in (("customer", CUSTOMER_DIR), ("sales_order", ORDER_DIR), ("interaction_event", EVENT_DIR)):
